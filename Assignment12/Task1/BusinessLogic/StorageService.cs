...
namespace Task1.BusinessLogic
{
    internal class StorageService<T> : IStorageService<T> where T : class, IGood
    {
        ...
        private readonly string _spoiledFilePath;

        private Func<string, ILogger, IEnumerable<T>> _readStorageFromFile;

        public event Action<string, T> SpoiledSupply;

        public StorageService(IStorage<T> storage, ILogger logger, string sourceFilePath, string destinationFilePath, string spoiledFilePath)
        {
            _storage = storage;
            _logger = logger;
            _sourceFilePath = sourceFilePath;
            _destinationFilePath = destinationFilePath;
            _spoiledFilePath = spoiledFilePath;

            _storage.SpoiledSupply += OnStorageSpoiledSupply;
        }
        public StorageService(IStorage<T> storage, ILogger logger, string sourceFilePath, string destinationFilePath, string spoiledFilePath, Action<string, T> spoiledSupplyHandler)
            : this (storage, logger, sourceFilePath, destinationFilePath, spoiledFilePath)
        {
            SpoiledSupply += spoiledSupplyHandler;
        }
        public double TotalMass => _storage.TotalMass;
        public double TotalPrice => _storage.TotalPrice;
        private void OnStorageSpoiledSupply(T obj)
        {
            SpoiledSupply?.Invoke(_spoiledFilePath, obj);
        }
